...
public enum Item {
	...
	public static void useCalc(Pokemon p, Pokemon[] box, Pokemon f, boolean display) {
		...
		if (calc == null) {
			...
			JPanel userLevelPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
			userLevelPanel.add(userLevel);
			calc.add(userLevelPanel, gbc);
			gbc.gridx++;
			...
			calcFrame.pack();
			installScaling(calcFrame, contentPanel);
	...
	private static final String BASE_PREF = "basePref";
	private static float calcScale = 1f;
	
	/** Sets a font at scale 1 and remembers it so later rescales stay accurate. */
	private static void setBaseFont(JComponent c, Font base) {
		c.putClientProperty(BASE_FONT, base);
		c.setFont(base.deriveFont(base.getSize2D() * calcScale));
	}
	
	private static void setBasePreferredSize(JComponent c, Dimension base) {
		c.putClientProperty(BASE_PREF, base);
		c.setPreferredSize(new Dimension(Math.round(base.width * calcScale),
				Math.round(base.height * calcScale)));
	}
	
	private static void applyScale(Component c, float scale) {
		if (c instanceof JComponent) {
			JComponent jc = (JComponent) c;
			Font base = (Font) jc.getClientProperty(BASE_FONT);
			if (base == null && jc.getFont() != null) {
				base = jc.getFont();
				jc.putClientProperty(BASE_FONT, base);
			}
			if (base != null) jc.setFont(base.deriveFont(base.getSize2D() * scale));
			Dimension basePref = (Dimension) jc.getClientProperty(BASE_PREF);
			if (basePref != null) {
				jc.setPreferredSize(new Dimension(Math.round(basePref.width * scale),
						Math.round(basePref.height * scale)));
			}
		}
		if (c instanceof Container) {
			for (Component child : ((Container) c).getComponents()) applyScale(child, scale);
		}
	}
	
	private static void installScaling(JFrame frame, Container root) {
		final Dimension base = frame.getSize(); // call after pack()
		frame.addComponentListener(new ComponentAdapter() {
			@Override public void componentResized(ComponentEvent e) {
				float s = Math.min((float) frame.getWidth() / base.width,
						(float) frame.getHeight() / base.height);
				calcScale = Math.max(0.7f, Math.min(s, 3f));
				applyScale(root, calcScale);
				root.revalidate();
			}
		});
	}
	...
	private static void updateMoves(Pokemon current, JGradientButton[] moves, JLabel[] damages, Pokemon foe, JLabel[] statLabels, JComboBox<Integer>[] stages,
		...
		for (int k = 0; k < moves.length; k++) {
			if (current.moveset[k] != null) {
				...
				setBaseFont(moves[k], new Font("Arial", Font.BOLD, 12));
				setBasePreferredSize(moves[k], new Dimension(120, 26));
				if (!current.getValidMoveset().contains(move)) {
					moves[k].setSolid(true);
					moves[k].setBackground(Color.GRAY);
					setBaseFont(moves[k], new Font("Arial", Font.PLAIN, 12));
				}
				...
				setBaseFont(damages[k], crittable ? new Font("Arial", Font.BOLD, 12) : new Font("Arial", Font.PLAIN, 12));
			} else {
